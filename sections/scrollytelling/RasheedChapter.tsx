...
const rasheed = familyData.members.find(m => m.id === "rasheed");
const rasheedChildren = (rasheed?.children || []).map(childId => {
...
export default function RasheedChapter() {
  ...
  return (
    <section id="chapter-3" ref={containerRef} className="relative min-h-[150vh] py-32 px-6 bg-transparent overflow-hidden flex flex-col items-center">
      
      {/* Background Depth */}
      <motion.div style={{ y: backgroundY }} className="absolute inset-0 z-0 pointer-events-none">
         <div className="absolute inset-0 opacity-[0.03] bg-[radial-gradient(circle_at_top_right,_#d4af37_0%,_transparent_60%)]" />
      </motion.div>
      ...
      <div className="max-w-5xl mx-auto z-10 relative w-full text-center mb-24">
        ...
        >
          As the eldest son, Mohd Rasheed established a household deeply rooted in compassion and care. Alongside Sayyada Khatoon, they nurtured a lineage defined by selfless service, culinary traditions, and an immense capacity to forgive. 
        </motion.p>
      </div>
